...
function buildTable(data){
  ...
  data.forEach((c,i)=>{
    ...
    row.innerHTML = `<td><input type="checkbox" class="selectBox" data-id="${c.id}"></td>
      <td>${i+1}</td><td>${c.nama}</td><td>${c.telepon}</td><td>${c.email}</td><td>${c.perusahaan}</td><td>${c.catatan}</td>`;
...
function showDetailPopup(c){
  detailView.innerHTML = `<div id="detailInfo">
    <b>Nama:</b> ${c.nama}<br><b>Telepon:</b> ${c.telepon}<br><b>Email:</b> ${c.email}<br>
    <b>Perusahaan:</b> ${c.perusahaan}<br><b>Catatan:</b> ${c.catatan}<br><br>
...
exportCSVBtn.addEventListener("click", async()=>{
  ...
  const csv = [["Nama","Telepon","Email","Perusahaan","Catatan"], ...data.map(c=>[c.nama,c.telepon,c.email,c.perusahaan,c.catatan])].map(e=>e.join(",")).join("\n");
...
copyAllBtn.addEventListener("click", async()=>{
  ...
  showToast("Semua kontak disalin!");
});
